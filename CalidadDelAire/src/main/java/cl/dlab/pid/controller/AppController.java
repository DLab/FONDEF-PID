...
public class AppController implements ApplicationContextAware {
	...
	public void shutdownContext() {
		try {
			TaskUtil.getInstance().shutdownTask();
	...
	{
		return getCalidadDelAirePorUfIdFechaInicioFechaTermino(ufId, fechaInicio, fechaTermino, "csv", "A");
	}
	@SuppressWarnings("rawtypes")
	@GetMapping("/getCalidadDelAirePorUfIdFechaInicioFechaTermino/{ufId}/{fechaInicio}/{fechaTermino}/{formato}/{db}")
	public @ResponseBody ResponseEntity getCalidadDelAirePorUfIdFechaInicioFechaTermino(@PathVariable Integer ufId, @PathVariable String fechaInicio, @PathVariable String fechaTermino, @PathVariable String formato, @PathVariable String db) throws Exception
	{
		long t = System.currentTimeMillis();
		try
		{
			DataBase database = db.toUpperCase().charAt(0) == 'H' ? DataBase.Historica : db.toUpperCase().charAt(0) == 'S' ? DataBase.ApiRest_SMA : DataBase.ApiRest;
			logger.info("getCalidadDelAirePorUfIdFechaInicioFechaTermino:" + ufId + ", fechaInicio:" + fechaInicio + ", fechaTermino:" + fechaTermino + ", formato:" + formato +", db:" + database);
			FileVO file = new ConsultaPorFecha().getFile(database, ufId, fechaInicio, fechaTermino, formato);
			if (file == null)
			{
