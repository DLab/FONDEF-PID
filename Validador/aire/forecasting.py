...
def LSTM(parametros, data, additionalData):
    ...
    if(data.shape[1] != n_features):
        return {'ERROR': 'Número de variables incompatible con la red neuronal. Se esperan {} variables'.format(n_features)}
    else:
        try:
            fin = pdata.index.get_loc(to_datetime(termino))
        except:
            return {'ERROR': 'Fecha {} no encontrada en los datos'.format(termino)}
    ...
    return date_range(to_datetime(termino), periods=largo_prediccion, freq='1H').strftime('%Y-%m-%d %H:%M:%S'), prediction + scaler.inverse_transform(np.array(forecast).reshape(-1,1))
